...
Кожна фіскальна інтеграція додається окремою карткою — кнопкою **+ Додати ВчасноКаса** або **+ Додати Checkbox**.

- **Мітка** — довільна назва картки, щоб відрізняти кілька підключень.
- **Токен** — для ВчасноКаса: токен з особистого кабінету.
- **PIN-код**, **Імʼя клієнта**, **Версія клієнта**, **Ліцензійний ключ** — для Checkbox; усі чотири поля обовʼязкові.
...
Зміни зберігаються кнопкою **Зберегти**.
