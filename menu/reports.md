...
![](../_media/reports-statistics.png ':size=420x180')
...
- **Тип звіту** — випадаючий список:
  ...
  - **Потребує уточнення** (FC-110) — список клієнтів у яких **не заповнено поле телефону**. Тільки телефон — інші поля (email, дата народження) у фільтрі не враховуються. Зручно для контролю якості бази, бо без телефону клієнт не отримуватиме Telegram-нагадування і пуш-сповіщення з мобільного додатку.
- **Графік** — кнопка переключення між таблицею і візуалізацією (де доречно).

**Експорт:**

- **CSV** — для всіх типів звіту.
- **PDF** — для типу **Клієнти**.

---

## 3.8.2 Зміни

![](../_media/reports-shifts-1.png ':size=420x140')
...
![](../_media/reports-shifts-2.png ':size=420x140')
...
![](../_media/reports-categories.png ':size=420x140')
...
![](../_media/reports-category-form.png ':size=300x320')
...
![](../_media/reports-coach-salary.png ':size=420x180')
...
- **Тренер** — імʼя тренера.
- **Тип ставки** — вказаний у профілі тренера (категорія може перевизначити це для конкретного тренування).
- **Динамічні колонки** — система групує всі тренування періоду за **категоріями**, і кожна категорія стає окремою колонкою. Значення в колонці залежить від типу ставки категорії:
  - **За людину** — `сумарна кількість клієнтів × baseRate`.
  - **За тренування** — `кількість проведених тренувань × baseRate`.
...
![](../_media/reports-admin-salary.png ':size=420x140')
