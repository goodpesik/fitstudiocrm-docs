<a href="javascript:void(0)" onclick="history.back()">⬅️ Назад</a>

[Повернутися на головну](/)

# 1.3.2.2 Налаштування

> Розділ **Налаштування** — це адмін-панель студії, розбита на вкладки. Доступ мають ролі **Супер Адмін**, **Адмін** і **Менеджер**.

![](../_media/admin-panel-tabs.png ":size=600x362")

**Вкладки:**

- [1. Користувачі](#користувачі) — керування акаунтами адмінів.
- [2. Налаштування](#налаштування) — операційні години, часовий пояс, правила студії.
- [3. Функції](#функції) — фічі і бізнес-правила (відміни, бонуси, бот, нагадування).
- [4. Інтеграції](#інтеграції) — фіскалізація (ВчасноКаса, Checkbox) та еквайринг.
- [5. Імпорт](#імпорт) — імпорт клієнтів з Excel.
- [6. Інформація](#інформація) — версія та допоміжна інформація.
- [7. Мобільний додаток](#мобільний-додаток) — налаштування застосунку клієнта.
- [8. Нагадування](#нагадування) — власні нагадування для працівників студії.
- [9. Категорії](#категорії) — категорії для розрахунку зарплати тренерів (перенесено зі Звітів).
- [10. Інформація про студію](#інформація-про-студію) — назва, логотип, опис, фотографії та контакти студії. Вкладка зʼявляється, коли увімкнено мобільний застосунок для клієнтів.

---

## Користувачі

> Список адмінів студії, додавання нових, редагування ролей, видалення.

![](../_media/admin-panel-users.png ":size=600x362")

Повний опис таблиці, запрошення нового користувача та ролей — на сторінці [Користувачі](/login/users). Тут наведено лише те, де ця вкладка знаходиться, щоб не тримати два описи одного екрана: вони неминуче розходяться.

---

## Налаштування

> Загальні параметри студії.

![](../_media/admin-panel-settings.png ":size=600x362")

> ⚠️ **Назву студії та логотип перенесено** на вкладку [Інформація про студію](#інформація-про-студію). Ця вкладка зʼявляється лише тоді, коли увімкнено мобільний застосунок для клієнтів.

### Назва студії

Текстове поле на вкладці **Інформація про студію**. Використовується у телеграм-боті та фіскальних чеках.

### Лого студії

> Завантаження або заміна логотипу. Підтримує JPG/PNG. Кнопка **Видалити фото** прибирає поточне лого.

### Операційні години студії

> Час початку і кінця робочого дня (вибір через time picker). Впливає на:

- Підказки у формі створення тренувань.
- Розрахунок зарплати адмінів (не може перевищувати тривалість зміни).
- Мобільний додаток (приховує бронювання поза цими годинами).

### Правила студії

> Текстове поле з підтримкою багаторядкового тексту. Відображається клієнтам у мобільному додатку (вкладка **Кабінет → Інформація → Правила**).

### Часовий пояс студії

> Випадаючий список з IANA-часовими поясами (за замовчуванням `Europe/Kyiv`). Впливає на ВСІ дати та звіти.

> ⚠️ Зміна часового поясу — операція з широким радіусом дії. Селект блокується якщо у студії є хоча б одне тренування — інакше дати у вже створених тренуваннях можуть «зсунутись». Спочатку треба видалити/обробити всі тренування, потім міняти TZ.

---

## Функції

> Бізнес-правила і фіча-флаги. Кожна група винесена у власну секцію з заголовком.

![](../_media/admin-panel-functions.png ":size=600x362")

### Мінімальний час до відміни тренування

- **Час до відміни тренування (год)** — число (за замовчуванням 6).
- Якщо клієнт має активний абонемент і скасовує тренування **менше ніж за вказану кількість годин** до початку — тренування **списується** з абонементу як **Не відвідано**.

### Мінімальна кількість клієнтів

- **Мін. к-сть** — число.
- Якщо налаштування > 0 і на тренуванні було менше клієнтів — тренеру зараховується мінімум (захист від невигідних малих груп).
- Доступно тільки для типу ставки **"Кількість людей на тренуванні"** (`PerPerson`).

### Знімати повторний запис після пропусків тренувань

- **Кількість пропусків** — число.
- Якщо клієнт має дублюючий запис на тренування і пропускає N послідовних занять (статус **Не відвідано**) — система автоматично знімає його з повторного запису.
- `0` або порожнє поле → вимикає функцію.

### Бонуси адміністраторів

> Налаштування бонусів, що нараховуються у звіті [Зарплати адмінів](/menu/reports#_3-8-5-зарплати-адмінів).

Кожен бонус — рядок з трьох полів:

- **Умова** — за що нараховується (наприклад, "Кожна продана картка", "Кожна закрита зміна").
- **Нарахування** — спосіб (фіксована сума / відсоток).
- **Тип клієнта** — фільтр (новий клієнт / усі).

Кнопки **➕ Додати бонус** і **🗑 Видалити** (іконка кошика) для кожного рядка.

### Створення персональних тренувань через бот

- **Тогл "Дозволити тренерам створювати персональні тренування через бот"** — вмикає функціонал.
- Якщо увімкнено, зʼявляється список **категорій**, для яких бот дозволяє створювати тренування. Тренер може створити персональне тренування тільки в межах цих категорій.
- Кнопка **+ Додати категорію** додає рядок з випадаючим списком категорій студії.

### Нагадувати про дні народження тренерів

- **Днів** — за скільки днів до ДН прийде сповіщення.
- `0` → функція вимкнена.

### Функції (додаткові тогли)

- **Повідомляти клієнтам про заміну тренера (Додаток)** — якщо для тренування призначено заміну, всі записані клієнти отримають push у мобільному додатку.
- **Враховувати максимальну к-сть людей при підрахунку зарплат** — обмежує зараховану тренеру к-сть клієнтів значенням `workoutCapacity`.
- **Закривати зміну автоматично** (закриється о 23:57) — згідно з законодавством, зміна по касі не може тривати понад 24 години. Автозакриття уникає людського фактора і дотримує цю вимогу.
- **Нараховувати борг за неявку на персональне (без абонемента) та зараховувати тренеру** — коли увімкнено: якщо персональному тренуванню виставлено статус **«Не відвідано»**, а в клієнта **немає активного персонального абонемента**, система автоматично створює клієнту **борг** на суму вартості тренування (ціна привʼязаного до категорії абонемента) і **зараховує тренеру** це тренування у зарплату. Спрацьовує на будь-якому виставленні «Не відвідано» (нічний авто-процес, пізня відміна, ручне виставлення адміном). Якщо в категорії тренування немає привʼязаного абонемента з ціною — борг не створюється.

---

### Danger Zone

Кнопка **Очистити тренування** незворотно скасовує всі ще не проведені тренування студії та зупиняє всі повторювані. Щоб підтвердити, потрібно ввести слово **DELETE** у полі підтвердження.

### Автовихід при бездіяльності

Кількість годин бездіяльності, після якої користувача автоматично виводить із системи. Значення `0` вимикає автовихід.

---

## Інтеграції

> Інтеграція з фіскальним реєстратором розрахункових операцій. Підтримуються **ВчасноКаса** та **Checkbox**. Кожна інтеграція додається окремою карткою.

![](../_media/admin-panel-prro.png ":size=600x362")

### Підключення

Кожна фіскальна інтеграція додається окремою карткою — кнопкою **Додати інтеграцію Вчасно Каса (фіскалізація)** або **Додати інтеграцію Checkbox (фіскалізація)**.

- **Назва** — довільна назва картки, щоб відрізняти кілька підключень.
- **Token** — для ВчасноКаса: токен з особистого кабінету.
- **Пінкод касира**, **Client Name**, **Client Version**, **License Key** — для Checkbox; усі чотири поля обовʼязкові.
- Перемикач **Активна** вмикає картку. Він спрацьовує лише після заповнення обовʼязкових полів, і одночасно активною може бути тільки одна картка кожного типу.

> ⚠️ Полів «Заголовок чека» та «Нижня частина чека» більше немає — їх прибрано разом із переходом на картки інтеграцій.

### Еквайринг Monobank

Окремою карткою додається еквайринг Monobank: токен, а для QR-оплат — вибір активного QR зі списку, який підтягується з Monobank. Можна додати кілька фізичних терміналів із власними назвами.

> ⚠️ Наразі це **лише налаштування**. Оплата в [Касі](/menu/pos) через Monobank ще не проводиться — картку можна заповнити, але на процес продажу вона поки не впливає.

### Пристрої

Перемикач і адреса локального помічника друку чеків (за замовчуванням `http://localhost:9999`) із перевіркою звʼязку. Налаштування діє **лише на цьому компʼютері**: на кожному робочому місці його треба вмикати окремо.

### Що змінюється при увімкненій фіскалізації

- Усі продажі (готівка/картка) **фіскалізуються** через ВчасноКаса; для кожної транзакції створюється фіскальний чек.
- При закритті зміни автоматично друкується Z-звіт.
- Поповнення особового рахунку також фіскалізується (як звичайна транзакція). Списання з особового рахунку — НЕ фіскалізується (бо гроші вже були зафіскалізовані при поповненні).

> ℹ️ Кнопки **X Звіт** і **Звіт за день** на сторінці [Каса](/menu/pos) доступні **коли відкрита зміна**, незалежно від фіскальної інтеграції. Без неї ці звіти показують підсумок руху коштів за зміну та за добу; з нею — додатково містять фіскальні дані.

---

## Імпорт

> Імпорт клієнтів з Excel-файлу.

[Детальніше про імпорт](/pages/setup#_4-7-2-імпорт-клієнтів-з-excel-файлу)

---

## Інформація

> Технічна інформація.

- **Версія** — поточна версія додатку.
- Дата та джерело останнього оновлення.
- Контакти підтримки.

---

## Мобільний додаток

> Інструменти для роботи з мобільним PWA-клієнтом студії.

> ℹ️ Опис додатку **очима користувача** (встановлення, вхід, запис на тренування, картка, сповіщення) — у розділі [7. Мобільний додаток](/mobile/mobile).

![](../_media/admin-panel-mobile.png ":size=600x362")

### Основні перемикачі

- **Використовувати мобільний застосунок для клієнтів** — головний перемикач. Щоб вимкнути його, потрібно ввести підтвердження **SWITCHOFF**: разом із застосунком клієнти втрачають запис онлайн.
- **Підтвердження запису гостя по SMS** — дозволяє людині без облікового запису підтвердити запис кодом у SMS, а не лише через Telegram.
- **Пропонувати оцінити заняття** — після відвіданого заняття клієнт отримує пропозицію поставити оцінку.

### Нагадування про візит

- **Нагадувати про візит** — за скільки годин до заняття надсилати нагадування. Значення `0` вимикає нагадування.

> ⚠️ Це налаштування діє **лише на клієнтів без встановленого застосунку**. Ті, хто користується застосунком, задають собі час нагадування самі. Повний опис обох каналів, нічної паузи та спільного ліміту SMS — у розділі [Нагадування про тренування](/pages/processing#_6-6-нагадування-про-тренування).
- Поруч показано, скільки SMS уже витрачено з місячного ліміту студії.
- **Текст SMS-нагадування** — власний текст студії. Дата й час підставляються автоматично, тому редагуються лише слова до і після них. Лічильник показує залишок із 57 символів: більше в одне SMS не вміщується, а кожне наступне оплачується окремо.

### Публічне посилання на студію

Посилання на публічну сторінку студії разом із QR-кодом. За ним будь-хто може переглянути розклад і записатися на заняття без облікового запису.

### Посилання + QR

- **Посилання на додаток** — URL встановлення PWA (за замовчуванням `https://app.fitshandler.com`). Клікабельне, відкривається в новій вкладці.
- **Завантажити QR код** — кнопка завантажує PNG з QR-кодом, що веде на це посилання. Можна надрукувати і показати клієнту у студії.
- **Завантажити баннер** — готовий до друку рекламний банер з QR-кодом і логотипом додатку.

### Лічильники користувачів

Інформаційний блок з кількістю зареєстрованих у додатку:

- **Зареєстровано клієнтів: N** — скільки клієнтів студії підключило мобільний додаток.
- **Зареєстровано тренерів: N** — скільки тренерів студії має активний акаунт у додатку.

### Надіслати повідомлення від студії

> Розсилка push-повідомлення усім користувачам додатку обраної аудиторії.

- **Виберіть тип повідомлення** — випадаючий список:
  - **Клієнтам** — push отримають усі клієнти.
  - **Тренерам** — push отримають усі тренери.
  - **Усім** — обом аудиторіям.
- **Текстовий редактор** — підтримує форматування (жирний/курсив/посилання). Має ліміт довжини; при перевищенні зʼявляється помилка "Повідомлення пусте або занадто довге".
- **Надіслати** — кнопка стає активною після вибору аудиторії та введеного тексту.

> ⚠️ Розсилка йде **лише через застосунок**. Клієнти без встановленого застосунку її не отримають — на відміну від нагадувань про візит, запасного каналу через Telegram чи SMS тут немає. Натискання миттєво ставить повідомлення в чергу для доставки усім пристроям.

---

## Нагадування

> Власні нагадування для працівників студії: разові або повторювані записи з датою, часом і текстом. Не плутати з нагадуваннями клієнтам про візит — ті налаштовуються на вкладці [Мобільний додаток](#мобільний-додаток).

---

## Інформація про студію

> Вкладка зʼявляється, коли увімкнено мобільний застосунок для клієнтів. Усе, що тут заповнено, клієнт бачить у застосунку в шапці студії та у вікні **«Про нас»**.

- **Назва студії** — використовується в застосунку, телеграм-боті та фіскальних чеках.
- **Логотип студії** — зображення, яке клієнт бачить поруч із назвою.
- **Про нас** — опис студії.
- **Фотографії** — до десяти світлин галереї.
- **Місто** — крім картки студії, використовується для фільтра за містом у переліку студій.
- **Телефон** і **Адреса** — контакти, які клієнт бачить у шапці студії та у вікні «Про нас».

Зміни зберігаються кнопкою **Зберегти інформацію**.

> ℹ️ Поки ці поля порожні, у застосунку показується лише назва й логотип: кнопка **«Про нас»** є завжди, але всередині буде повідомлення, що інформації поки немає.

---

## Категорії

> Вкладка **Категорії** (раніше була у [Звітах](/menu/reports)). Категорії використовуються для розрахунку зарплати тренерів і опційно для звʼязки з абонементом.

**Таблиця колонки:**

| Колонка                     | Опис                                                                                                                                                           |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Назва**                   | Імʼя категорії (наприклад "Групові", "Персональні", "Дитячі").                                                                                                 |
| **Базова ставка категорії** | Сума у грн, що йде у формулу зарплати.                                                                                                                         |
| **Тип ставки**              | **За людину** (`PerPerson`) — рахується кількість клієнтів і множиться на ставку. **За тренування** (`PerWorkout`) — рахується кількість проведених тренувань. |
| **Абонемент**               | Опційний звʼязок з конкретним абонементом (для категорій під персональні плани).                                                                               |
| **Дії**                     | Редагувати / Видалити.                                                                                                                                         |

**Пошук** — поле над таблицею, шукає по назві.

### Додати / Редагувати категорію

> Кнопка **+ Додати категорію** відкриває модалку. Така ж модалка — при натисканні на **Редагувати** у рядку.

**Поля:**

- **Назва** — обовʼязкове.
- **Базова ставка** — обовʼязкове, число у грн.
- **Тип ставки** — **За людину** / **За тренування**.
- **Абонемент** — привʼязка категорії до конкретного абонементу. Поле **опційне для типу "За людину"**, але **обовʼязкове для типу "За тренування"** (позначене зірочкою у формі). Логіка: при PerWorkout зарплата за категорію залежить від конкретного плану клієнта; без абонементу неможливо однозначно порахувати.

> ℹ️ Якщо обрано **За тренування** без абонементу — кнопка **Зберегти** буде неактивна. Спершу створіть або оберіть відповідний абонемент.

---

<a href="javascript:void(0)" onclick="history.back()">⬅️ Назад</a>

[Повернутися на головну](/)
